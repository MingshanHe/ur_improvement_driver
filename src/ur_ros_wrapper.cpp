...
protected:
	UrDriver robot_;
	KinematicChainControllerBase kinematicchaincontrollerbase;
...
public:
	RosWrapper(std::string host, int reverse_port) :
		...
		io_flag_delay_(0.05), joint_offsets_(6, 0.0), gripper_width(0.0), gripper_count(0), gripper_flag(false)
	{
		//! Cartesian Velocity Control
		cartesian_velocity_control = false;
		ros::param::get("~cartesian_velocity_control", cartesian_velocity_control);
		if(cartesian_velocity_control)
		{
			kinematicchaincontrollerbase.init(nh_);
			ik_vel_solver_.reset(new KDL::ChainIkSolverVel_pinv_givens(kinematicchaincontrollerbase.kdl_chain_));
			fk_vel_solver_.reset(new KDL::ChainFkSolverVel_recursive(kinematicchaincontrollerbase.kdl_chain_));
			fk_pos_solver_.reset(new KDL::ChainFkSolverPos_recursive(kinematicchaincontrollerbase.kdl_chain_));
			command_cart_vel_sub = nh_.subscribe("/ur5/cartesian_velocity_controller/command_cart_vel", 5, &RosWrapper::command_cart_vel, this);
			gripper_set = nh_.subscribe("/ur5/joint_states",5,&RosWrapper::gripper, this);
			// Variable init
			kinematicchaincontrollerbase.joint_msr_.resize(6);
			q_dt_cmd_.resize(6);
		...
		if (robot_.start())
		{
			...
			gripper_sub_ = nh_.subscribe("ur_driver/gripper", 1, &RosWrapper::gripperInterface, this);
...
private:
	...
	void gripperInterface(const std_msgs::Float64::ConstPtr& msg)
	{
		std::cout<<"In gripper Interface"<<std::endl;
		gripper_flag = true;
	...
	void rosControlLoop()
	{
		...
		realtime_tools::RealtimePublisher<geometry_msgs::WrenchStamped> wrench_pub( nh_, "wrench", 1);

		//!Cartesian Velocity Control
		realtime_tools::RealtimePublisher<cartesian_state_msgs::PoseTwist> ee_state_pub_( nh_, "/ur5/cartesian_velocity_controller/ee_state", 1);
		realtime_tools::RealtimePublisher<trajectory_msgs::JointTrajectory> vel_command_(nh_, "/ur5/cartesian_velocity_controller/joint_speed", 1);
		...
		while (ros::ok()) {
			...
			//Publish Wrench State
			std::vector<double> tcp_force = robot_.rt_interface_->robot_state_->getTcpForce();
			if ( wrench_pub.trylock())
			{
				wrench_pub.msg_.header.stamp = ros_time;
				wrench_pub.msg_.wrench.force.x = tcp_force[0];
				wrench_pub.msg_.wrench.force.y = tcp_force[1];
				wrench_pub.msg_.wrench.force.z = tcp_force[2];
				wrench_pub.msg_.wrench.torque.x = tcp_force[3];
				wrench_pub.msg_.wrench.torque.y = tcp_force[4];
				wrench_pub.msg_.wrench.torque.z = tcp_force[5];
			...
			if(cartesian_velocity_control)
			{
				fk_vel_solver_->JntToCart(kinematicchaincontrollerbase.joint_msr_, x_dot_);
				fk_pos_solver_->JntToCart(kinematicchaincontrollerbase.joint_msr_.q, x_);
				...
				for (unsigned int i = 0; i < joint_msg.position.size(); i++)
				{
					...
					kinematicchaincontrollerbase.joint_msr_.q(i)   = joint_msg.position[i];
					kinematicchaincontrollerbase.joint_msr_.qdot(i) = joint_msg.velocity[i];
				}
				ik_vel_solver_->CartToJnt(kinematicchaincontrollerbase.joint_msr_.q, x_dt_des_, q_dt_cmd_);
				if (vel_command_.trylock())
				{
					double acc = 100;
					robot_.setSpeed(q_dt_cmd_(0),q_dt_cmd_(1),q_dt_cmd_(2),q_dt_cmd_(3),q_dt_cmd_(4),q_dt_cmd_(5), acc);
					vel_command_.msg_.header.frame_id = "base_link";
					vel_command_.msg_.joint_names.resize(6);
					vel_command_.msg_.points.resize(1);
					vel_command_.msg_.joint_names[0] = "shoulder_pan_joint";
					vel_command_.msg_.joint_names[1] = "shoulder_lift_joint";
					vel_command_.msg_.joint_names[2] = "elbow_joint";
					vel_command_.msg_.joint_names[3] = "wrist_1_joint";
					vel_command_.msg_.joint_names[4] = "wrist_2_joint";
					vel_command_.msg_.joint_names[5] = "wrist_3_joint";

					vel_command_.msg_.points[0].velocities.resize(6);

					vel_command_.msg_.points[0].velocities[0] = q_dt_cmd_(0);
					vel_command_.msg_.points[0].velocities[1] = q_dt_cmd_(1);
					vel_command_.msg_.points[0].velocities[2] = q_dt_cmd_(2);
			...
			if(gripper_flag)
			{
				...
				std::cout<<"sleep ros control thread for 400 ms"<<std::endl;
				gripper_flag = false;
			}
		}
	}
	...
	void publishRTMsg()
	{
		...
		while (ros::ok())
		{
			...
			std::cout<<"joint states publish"<<std::endl;
			std::vector<double> tcp_force = robot_.rt_interface_->robot_state_->getTcpForce();
	...
	void publishMbMsg()
	{
		...
		while (ros::ok())
		{
			...
			{
				std::this_thread::sleep_for(std::chrono::milliseconds(400));
				std::cout<<"sleep publishMbMsg thread for 400 ms"<<std::endl;
				gripper_flag = false;
			}
		}
	}
	...
	void gripper( const sensor_msgs::JointStateConstPtr &msg)
	{
		if (gripper_flag)
		{
			std::cout<<"set gripper width: "<<gripper_width<<std::endl;
			robot_.setgripper(gripper_width);
		}
	}
};
...
int main(int argc, char **argv) {
	...
	if ((ros::param::get("~reverse_port", reverse_port))) {
		if((reverse_port <= 0) or (reverse_port >= 65535)) {
			print_warning("Reverse port value is not valid (Use number between 1 and 65534. Using default value of 30003");
			reverse_port = 30003;
		}
	} else
		reverse_port = 30003;
